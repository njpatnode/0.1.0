...
class result_function(object):
    ...
    def lin_reg(self, dataview.df, x_variables, y_variable=None, test_split=0.2):
        if y_variable is not None:
            ...
            fig_html = "Error: No y variable was provided."           
        return fig_html
    def log_reg(self, dataview.df, x_variables, y_variable, test_split=20, weights=6):
        if y_variable is not None:
            x_variables = dataview.df[[x_variables]]
            fig, ax = plt.subplots()
            x_train, x_test, y_train, y_test = train_test_split(x_variables, dataview.df[y_variable], test_size= (int(test_split)/100))
